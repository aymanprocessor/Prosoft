...
namespace ProSoft.UI.Areas.Treasury.Controllers
{
    ...
    public class TransferPermissionController : Controller
    {
        private readonly IAccSafeCashRepo _accSafeCashRepo;
        private readonly IUserCashNoRepo _userCashNoRepo;
        public TransferPermissionController(IAccSafeCashRepo accSafeCashRepo, IUserCashNoRepo userCashNoRepo)
        {
        ...
        public async Task<IActionResult> Add_TransferPermission(AccSafeCashEditAddDTO accSafeCashDTO)
        {
        ...
        //Get Edit
        public async Task<IActionResult> Edit_TransferPermission(int id)
        {
            var userCode = int.Parse(User.Claims.FirstOrDefault(c => c.Type == "User_Code").Value);
            ViewBag.userCode = userCode;
            ViewBag.branchId = int.Parse(User.Claims.FirstOrDefault(c => c.Type == "U_Branch_Id").Value);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit_TransferPermission(int id, AccSafeCashEditAddDTO accSafeCashDTO)
        {
            if (ModelState.IsValid)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete_TransferPermission(int id)
        {
            ...
            if (hasRelatedData)
            {
                ...
                return RedirectToAction("Index", "TransferPermission", new { docType = "SFTOT", flagType = "oneANDtwoAndthree", errorMessage });
            }
