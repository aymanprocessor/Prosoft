...
namespace ProSoft.Core.Repositories.Accounts
{
    public class AccTransDetailRepo : Repository<AccTransDetail, int>, IAccTransDetailRepo
    {
        ...
        {
            List<AccTransDetail> accTransDetails = await _Context.AccTransDetails.Where(obj => obj.TransType == journalCode.ToString() && obj.TransId==transId)
                .OrderBy(obj => obj.TransSerial).ToListAsync();
